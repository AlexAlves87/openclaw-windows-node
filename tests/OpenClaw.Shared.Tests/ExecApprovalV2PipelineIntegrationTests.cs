...
public class ExecApprovalV2PipelineIntegrationTests : IDisposable
{
    ...
    public async Task AttackCommand_ReachesDialogSanitized_AllowOnceProducesAllow()
    {
        ...
        Assert.DoesNotContain(U(Rlo), captured!.CommandText);
    ...
    public async Task SpoofedCwd_ReachesDialogSanitized()
    {
        ...
        Assert.DoesNotContain(U(Zwsp), captured.CwdText!);
    ...
    // Even if the user clicks Allow on a shell-wrapper command, the pipeline fails closed
    // after approval instead of executing it: a shell re-parses its argument tail, so the
    // executed effect could differ from the approved text. The approval never yields a
    // runnable payload.
    [Fact]
    public async Task ShellWrapperCommand_ApprovedInDialog_FailsClosedWithNoExecution()
    {
        var attack = Req("""{"command":["cmd","/c","echo","SAFE&calc.exe"]}""");
        var result = await MakeCoordinator(dialog: _ => ExecApprovalPromptOutcome.AllowOnce)
            .HandleAsync(attack, "shell-1");
